...
CLIENT_OUTPUT_DIR = ROOT / "edge_service" / "dist" / "EdgeServiceClient" / "output"
CLIENT_CORE_INTERNAL_DIR = ROOT / "edge_service" / "dist" / "EdgeServiceClient" / "core" / "_internal"
USE_CLIENT_RUNTIME_CONTEXT = True

def _bootstrap_sdk_dir() -> Path:
    override = str(os.getenv("ADHOC_HIK_SDK_DIR") or os.getenv("EDGE_HIK_SDK_DIR") or "").strip()
    if override:
        return Path(override).resolve()
    candidates = [
        ROOT / "edge_service" / "dist" / "EdgeServiceClient" / "core" / "_internal" / "sdk" / "download",
        ROOT / "edge_service" / "dist" / "EdgeServiceClient" / "sdk" / "download",
        ROOT / "edge_service" / "sdk" / "download",
        ROOT / "sdk" / "download",
    ]
...
if USE_CLIENT_RUNTIME_CONTEXT and CLIENT_OUTPUT_DIR.exists():
...
WEB_CHANNEL_NUM = 8
TIME_RANGE_TEXT = "2026-06-06 15:20 ~ 2026-06-06 18:10"
...
SDK_DIR = ""
OUTPUT_LABEL = "adhoc"
OUTPUT_ROOT = ROOT / "adhoc_download_runs"
...
def _print_config(start_dt: datetime, end_dt: datetime, work_dir: Path, log_path: Path) -> None:
    ...
    print("===== adhoc official flow config =====")
    ...
    print(f"nvr_device_id={NVR_DEVICE_ID}")
...
def main() -> int:
    ...
    log_path = work_dir / "adhoc_download_repair_merge_official_flow.latest.log"
    ...
    try:
        ...
        print(f"log_file={log_path}")
        print("[OK] adhoc official flow succeeded")
        return 0
